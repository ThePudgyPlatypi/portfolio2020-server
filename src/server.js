...
app.post('/api/pieces/:title/update-piece', async (req, res) => {
    withDB( async (db) => {
        const pieceName = req.params.name;
        const pieceInfo = await db.collection('pieces').findOne({name: pieceName});
        await db.collection('pieces').updateOne({title: pieceName }, {
            '$set' : {
                name: req.body.name,
...
app.post('/api/pieces/:id/:key/update-piece', async (req, res) => {
    withDB( async (db) => {
        ...
        await db.collection('pieces').updateOne({_id: o_id }, {
            '$set' : dynamicSet
        });
        const updatedPieceInfo = await db.collection('pieces').findOne({_id: o_id});
        res.status(200).json(updatedPieceInfo);
    }, res);
});
